...
const BackgroundChooserItemLoader = () => {
    return (
        <div
            className={
                "absolute left-0 top-0 z-10 flex h-full w-full items-center justify-center bg-slate-500/50"
            }
        >
            <Loader2 className="mr-1 animate-spin text-white" />
        </div>
    );
};
const BackgroundChooser = ({
    ...
    return (
        <Chooser className={"grid grid-cols-4 gap-x-2 sm:gap-x-4"} {...props}>
            <ChooserItem
                title={"Flat"}
            ...
            >
                <div
                    ...
                    style={flatPreview}
                >
                    {loading && props.value === 1 && (
                        <BackgroundChooserItemLoader />
                    )}
                </div>
            </ChooserItem>
            <ChooserItem
            ...
            >
                ...
                {videoPreview ? (
                    ...
                    <MaterialSymbolsVideoCameraBackOutline
                        className={"h-10 w-10 md:h-14 md:w-14"}
                    />
                )}
            </ChooserItem>
...
export const FormBackgroundChooser = ({
    name,
    control,
    ...props
}: ControlledProp) => {
...
export const BackgroundChooserSkeleton = ({}: InputProps) => {
    return (
        <ChooserSkeleton className={"grid grid-cols-4"}>
            <ChooserItemSkeleton
                className={"col-span-1 aspect-[132/204] w-full min-w-full"}
            />
            <ChooserItemSkeleton
