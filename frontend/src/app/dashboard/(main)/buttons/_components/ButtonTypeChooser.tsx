...
import InputWrapper, {InputWrapperSkeleton} from "@/components/ui/input-wrapper";
import ButtonsItem, {ButtonsItemSkeleton} from "@/app/dashboard/(main)/buttons/_components/ButtonsItem";
...
export const FormButtonTypeChooser = ({name, control, ...props}: ControlledProp) => {
...
export const ButtonTypeChooserSkeleton = () => {
    return <div className={'flex flex-col w-full gap-y-5'}>
        {buttonsStyles.map((item) => <InputWrapperSkeleton key={item.name}>
