...
import Link, {ActiveLinkProps} from "@/components/ui/link";

type Props = {
    children?: React.ReactNode,
} & ActiveLinkProps;

const AppLogoIcon = ({href='/', className = 'flex h-[30px] min-h-[30px]' ,...props}: Props) => {
    return <Link className={className} href={href} {...props}>
        <svg className={'h-full'} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 25">
            <path
